...
class IrrigationSystem(HomeAccessory):
    ...
    async def async_call_service_and_wait(
        ...
        """Call a service and restore the affected zone if it fails."""
        success = await super().async_call_service_and_wait(
            domain, service, service_data, value
        )
        if not success and (
            zone := self.zones.get(service_data.get(ATTR_ENTITY_ID, ""))
        ):
